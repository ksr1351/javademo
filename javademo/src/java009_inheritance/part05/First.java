 ...
 *   2) 매개변수의 타입, 갯수, 순서가 일치해야한다. (= 오버로딩의 조건과 같음)
 *   3) 리턴타입이 동일해야한다.
 *   4) 접근제어자(제한자)는 부모클래스의 메소드와 같거나 더 넓은 범위어야 한다.
 *      private < default < protected < public 
 *   5) 부모클래스 메소드에서 throws가 선언되어 있으면 자식클래스의 메소드에서는 같거나 더 구체적이면 된다. (상속받은 것을 쓰면 됨)
 *      Exception < RuntimeException < NullPointerException
 *      
 *      
 * super
 * 1. 자식클래스에서 부모클래스를 호출할 때 사용한다.
 * 2. super.멤버변수
 * 	  super.메소드()
 *    super() => 생성자
 *    
